...
Entwurfsentscheidung dieses Moduls:
    ...
    <pfad>/
        ...
        vorschau/               <- schnelle PNG-Vorschau, Textstatistik und
                                    zustand_lesbar.json (Siedlungslisten und
                                    Skalare im Klartext) für Menschen,
                                    ebenfalls nicht zurückgelesen
...
ZUSAMMENFÜHRUNG MIT DER ZWEITEN FASSUNG (2026-09-22)
-----------------------------------------------------

Ticket #38 wurde zweimal unabhängig gelöst: einmal hier, einmal direkt in
`gui/tabs/overview_tab.py` (Commit `c571db4`, Dateien `welt_arrays.npz` +
`welt_zustand.json`). Übernommen wurde diese Fassung, weil die andere beim
Laden nur zwei der sieben Kategorien (terrain, geology) in den lebenden
`DataLODManager` zurückschrieb - die übrigen fünf wurden zwar gelesen und
zurückgegeben, landeten aber nie im laufenden Programm.

Was aus der anderen Fassung übernommen wurde, damit nichts verloren geht:

* `OverviewTab.welt_backen()`/`welt_laden()` bleiben unter ihren Namen
  erhalten und leiten nur noch hierher weiter; `WeltLadenFehler` ist von
  dort weiterhin importierbar (es ist dieselbe Klasse, siehe unten).
* `OverviewTab.welt_laden()` gibt weiterhin `{kategorie: {feld: wert}}`
  zurück - jetzt aber für ALLE sieben Kategorien, weil es die Antwort aus
  `get_all_data()` nach dem Zurückschreiben zusammenstellt.
* Die lesbare Klartext-Fassung der Nicht-Array-Felder (früher
  `welt_zustand.json`) liegt jetzt als `vorschau/zustand_lesbar.json`
  daneben - reine Ausgabe, kein zweiter Speicherpfad.
* Der `godot/`-Unterordner und `welt_manifest.json` gab es in beiden
  Fassungen und bleibt.
"""
...
def _lesbarer_zustand(data_lod_manager) -> Dict[str, Any]:
    """
    Funktionsweise: Baut eine JSON-taugliche Fassung des Weltzustands - Arrays
    nur als Form/Datentyp (ein 1024x1024-Feld als Zahlenliste waere unlesbar
    und riesig), alles andere im Klartext. Dataclass-Eintraege in Listen
    (`Location` aus core/settlement_generator.py) werden ueber
    `dataclasses.asdict()` aufgeloest, weil sie sonst nicht serialisierbar
    waeren.
    Aufgabe: Ersetzt die frueher von OverviewTab geschriebene
    `welt_zustand.json` (siehe Aufrufstelle in welt_backen()).
    Return: {kategorie: {feld: lesbarer Wert}}
    """
    import dataclasses
    ...
    for kategorie in KATEGORIEN:
        ...
        for key, wert in data.items():
            if isinstance(wert, np.ndarray):
                eintrag[key] = {"array": True, "shape": list(wert.shape),
                                "dtype": str(wert.dtype)}
                ...
                eintrag[key] = dataclasses.asdict(wert)
            else:
                eintrag[key] = wert
        lesbar[kategorie] = eintrag
    return lesbar
...
def welt_backen(pfad: str, data_lod_manager, parameter_manager=None) -> Dict[str, Any]:
    ...
    for kategorie in KATEGORIEN:
        ...
        if not data:
            manifest["kategorien"][kategorie] = {"vorhanden": False, "keys": [], "lod": 0}
            continue
        ...
        # Das LOD-Level MIT sichern. get_all_data() liefert immer den Snapshot
        # des aktuellen (hoechsten) LOD - schriebe welt_laden() ihn pauschal
        # auf LOD 1 zurueck, laege er unterhalb eines evtl. schon vorhandenen
        # hoeheren LOD und waere fuer jeden Leser unsichtbar, ohne dass etwas
        # auffiele. Genau dieser stille Teilzustand ist der Fehlertyp, vor dem
        # CLAUDE.md warnt.
        manifest["kategorien"][kategorie] = {
            "vorhanden": True,
            "keys": sorted(data.keys()),
            "lod": int(data_lod_manager.get_current_lod_level(kategorie)),
        }
    ...
    # Lesbare Fassung des Nicht-Array-Zustands (Siedlungs-/Landmark-/
    # Roadsite-Listen, Skalare). Uebernommen aus der frueheren Fassung in
    # gui/tabs/overview_tab.py, die diese Felder als `welt_zustand.json` im
    ...
    # Siedlungsdaten ist, siehe docs/SIEDLUNGEN_ENTWURF.md §6.
    ...
    try:
        vorschau_dir = os.path.join(pfad, _VORSCHAU_ORDNER)
        os.makedirs(vorschau_dir, exist_ok=True)
        lesbar = _lesbarer_zustand(data_lod_manager)
        with open(os.path.join(vorschau_dir, "zustand_lesbar.json"), "w",
                  encoding="utf-8") as f:
            json.dump(lesbar, f, ensure_ascii=False, indent=2, default=str)
        manifest["zustand_lesbar"] = True
    except Exception as exc:
        logger.warning("Lesbare Zustandsfassung in welt_backen() fehlgeschlagen: %s", exc)
        manifest["zustand_lesbar"] = False
...
def welt_laden(pfad: str, data_lod_manager, parameter_manager=None) -> Dict[str, Any]:
    ...
    for kategorie, info in manifest.get("kategorien", {}).items():
        ...
        if "lod" not in info:
            raise WeltLadenFehler(
                f"Kategorie '{kategorie}': das Manifest nennt kein LOD-Level. "
                f"Diese Welt stammt aus einer aelteren Fassung von "
                f"welt_backen() - bitte neu backen. (Ein Vorgabewert waere "
                f"hier ein stiller Ersatzpfad: die Daten laegen auf dem "
                f"falschen LOD und blieben unsichtbar.)")
        ziel_lod = max(1, int(info["lod"]))
        data_lod_manager.set_all_data(kategorie, data, lod_level=ziel_lod)
        ...
        # Zweite, schaerfere Nachpruefung: sind die zurueckgelesenen Werte
        # WIRKLICH die soeben geschriebenen (Identitaet, nicht nur gleicher
        # Name)? Hat der Manager fuer diese Kategorie bereits ein HOEHERES LOD
        # als die geladene Welt, liefert get_all_data() weiterhin jenes alte
        # LOD - gleiche Key-Namen, andere Daten. Die Pruefung oben koennte das
        # nicht sehen; das Programm zeigte dann die alte Welt und meldete
        # "geladen". Genau der Fehlertyp aus CLAUDE.md ("zwei Enden der Kette
        # gegeneinander messen").
        verschattet = sorted(k for k in erwartete_keys
                             if wiederhergestellt.get(k) is not data[k])
        if verschattet:
            raise WeltLadenFehler(
                f"Kategorie '{kategorie}': die geladenen Daten liegen auf LOD "
                f"{ziel_lod}, der Manager liefert aber weiterhin andere Werte "
                f"(aktuelles LOD {data_lod_manager.get_current_lod_level(kategorie)}) "
                f"fuer: {verschattet}. Die geladene Welt wuerde unsichtbar "
                f"bleiben. Vor dem Laden den vorhandenen Stand verwerfen "
                f"(DataLODManager.invalidate_cache_lod('{kategorie}') bzw. ein "
                f"frischer Manager).")
