...
# (siehe managers/calculator_graph.py, 34 Calculator-Knoten) ersetzt -
...
class GenerationOrchestrator(QObject):
    ...
    def __init__(self, data_lod_manager, shader_manager=None):
        ...
        # managers/calculator_graph.py). executors werden nur für den
    ...
    def get_downstream_generators(self, generator_type: GeneratorType) -> Set[GeneratorType]:
        ...
        """
        return self._collect_downstream(generator_type, set())

    def _collect_downstream(self, generator_type: GeneratorType,
                            besucht: Set[GeneratorType]) -> Set[GeneratorType]:
        """
        Rekursionskern mit ZYKLUSSCHUTZ.

        Der Generator-Baum darf Kreise enthalten, auch wenn der KNOTEN-Graph
        keine hat - er ist eine Vergroeberung. Beispiel vom 2026-07-29: die
        Kante weather.temperature <- biome.preseed_hint ist auf Knotenebene
        zyklenfrei (preseed_hint braucht nur terrain und erosion.slope), auf
        Generatorebene entsteht daraus aber
        weather <- biome <- water <- weather.
        Ohne `besucht` lief diese Methode dann endlos; in der laufenden App
        sichtbar als "Unhandled exception in request_generation: maximum
        recursion depth exceeded", direkt beim ersten Generierungs-Tick.
        """
        downstream = set()
        for dependent_type, dependencies in self.dependency_tree.items():
            if generator_type in dependencies and dependent_type not in besucht:
                besucht.add(dependent_type)
                downstream.add(dependent_type)
                downstream.update(self._collect_downstream(dependent_type, besucht))
        return downstream
...
class CalculatorThread(QThread):
    ...
    Aufgabe: Background-Ausführung von generator_instance._calc_<name>() ohne
        ...
        managers/calculator_graph.py). Die _calc_*-Methode liest ihre
