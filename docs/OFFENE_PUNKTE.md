...
| **[x]** | **13.5** | **Vektordaten exportieren: Fluesse, Wege, Grundstuecksgrenzen, Siedlungen**. Liegen bereits als Linienzuege/Polygone vor (`roads`, `sea_roads`, Flussnetz, `plot_edges`, `settlement_list`) - als JSON-Punktlisten in Weltkoordinaten ausgeben. In Godot daraus Splines: Wege als Decal oder Mesh, Fluesse als Wassermesh entlang der Spline, Grenzen als Zaun/Linie. Meeresoberflaeche braucht keine Daten, nur eine Ebene auf Hoehe 0 mit Wasser-Shader. | 2 | **VOLLSTAENDIG ERLEDIGT 2026-09-17 (Ticket #37):** `vektordaten()` schreibt `vektor.json` mit Wegen, Seewegen, Grundstuecksgrenzen, Ortslagen UND jetzt auch Fluessen in METERN. Der Knotengraph (`punkte`/`eltern`/`strahler`/`flaeche`) ueberlebt seit diesem Ticket die Rasterung in `_weltfluesse()` (`core/terrain_generator.py`, neues `TerrainData.river_graph`) und wird ueber alle drei Speicherstufen bis zum Export durchgereicht (eigener Smoke-Test `tests/smoke_test_flussgraph_propagation.py`). Der Export baut daraus maximale Linienzuege zwischen "Astpunkten" (Quellen/Muendungen/Zusammenfluessen) statt eine Kante je Zwei-Punkte-Segment - `_flusslinien_aus_graph()` in `gui/utils/map_export.py`, geprueft in `tests/smoke_test_flussexport_vektor.py` (Abdeckung, Zusammenhang, striktes RFC-8259-JSON). Isolierte Einzelpunkte (Quelle = eigene Muendung, kein Nachbar) werden bewusst ausgelassen, das sind keine Fluesse. **Offen:** kein Godot-Binary in dieser Umgebung verfuegbar, daher kein echter Einlesetest moeglich - `allow_nan=False` beim JSON-Schreiben ist der staerkste hier pruefbare Ersatzbeleg.
