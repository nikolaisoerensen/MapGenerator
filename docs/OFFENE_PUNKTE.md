...
| **[x]** | **10.6** | Erosionskette `EROSION_AKTIV` — *Nutzer 2026-08-06: "vorerst nein"*, dann **doch eingeschaltet am 27.08.2026** (`gui/config/value_default.py:1088`, `True`). Die Zeile stand bis zum 16.09.2026 offen und behauptete damit weiter, die Kette sei aus. Sie ist an: drei Erosionstests sind rot, ihre Ursache ist ungeklaert (`docs/TESTBERICHT.md`, Abschnitt 3 — dort korrigiert von "vier" auf "drei", Ticket #63), und der Paritaetsbruch Faktor 385 zwischen GPU und CPU steht damit mitten im laufenden Betrieb statt vor einer Reaktivierung. **Nachtrag 2026-09-18 (Ticket #30):** der letzte Halbsatz ist selbst veraltet — der Faktor-385-Paritaetsbruch war schon am 27.08.2026 behoben (zu grobes GPU-Meldeintervall, kein Rechenfehler) und am 16.09. sowie erneut am 18.09. mit echter GPU bestaetigt gruen. Siehe `docs/SITZUNGSLOG.md`, Eintrag 2026-09-18, fuer die Messung und den einzigen noch offenen (schmalen) Rest. | — |
| [ ] | 10.7 | **CPU-Erosionspfad: ein schmaler ungesicherter Rueckfall bleibt** *(Ticket #30, gemessen 2026-09-18)*. Bei 1024 px braucht ein vollstaendiger CPU-Lauf 8-10 Minuten gegen 8-16 Sekunden auf der GPU (Faktor 37-65x) — im Normalbetrieb kein Problem, weil `HydraulicFieldSimulator.simulate()` bei fehlender GPU oberhalb `MAX_CPU_RESOLUTION` (256 px) sofort laut mit `ValueError` verweigert (`core/erosion_generator.py`, Zeile ~606). Die Sperre greift aber NICHT, wenn eine GPU zwar vorhanden ist, ein Rechen-Abschnitt aber mitten im Lauf fehlschlaegt oder den 30-s-Timeout von `GPUWorker.submit()` reisst (`_simulate_gpu()` gibt dann `None` zurueck, `simulate()` faellt danach ungeprueft in die CPU-Schleife). Das ist kein Paritaets- oder Rechenfehler (die Physik stimmt exakt), sondern eine fehlende Groessenpruefung genau in diesem einen Fallback-Zweig — bei 1024 px wuerde daraus ein 1,5-2-Stunden-Lauf, der wie ein Haenger aussieht statt sauber abzubrechen. Absichtlich NICHT in derselben Sitzung behoben (Vorgabe: keine Aenderung an der Erosionsberechnung ohne Ruecksprache); die naheliegende Behebung waere dieselbe `MAX_CPU_RESOLUTION`-Pruefung auch in diesem Zweig. | 0.5 |
