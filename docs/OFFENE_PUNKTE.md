...
und `docs/archiv/2026-08-04_INTEGRATIONSPLAN.md` an Aufgaben stammte.
    ...
    [x]  erledigt, gemessen UND am laufenden Programm bestaetigt
    [!]  gebaut und gemessen, aber noch nicht gesehen — die Pruefung steht
         in `docs/PRUEFLISTE.md`, dort steht auch woran man sie erkennt
    [ ]  offen
    [~]  teilweise / mit Vorbehalt — die Einschraenkung steht jeweils im Text

**`[x]` heisst seit dem 2026-09-23 mehr als vorher.** Vorher hiess es "erledigt
und gemessen", und gemessen heisst bei uns fast immer headless — ein Skript hat
Zahlen gerechnet. Fuer alles, was nur im Bild sichtbar ist, war das zu wenig.
Dafuer gibt es jetzt `[!]`; der Ablauf steht in `CLAUDE.md`, Abschnitt "ein
Ticket ist erst fertig, wenn es geprueft wurde".
...
| | Punkte |
|---|---:|
| **[x] erledigt, gemessen und bestaetigt** | **108** |
| [!] gebaut, Pruefung am Programm offen | 4 |
| [~] teilweise, Einschraenkung benannt | 17 |
| [ ] offen | 54 |
| **Summe** | **183** |

*(Zuletzt am **2026-09-23** MASCHINELL nachgezaehlt (per Regex ueber die
Markenspalte, nicht fortgeschrieben) - die Vorgaengerzahl 83/21/43 = 147 stammte
vom 2026-08-16 und lag 36 Punkte zu niedrig, weil seither eingetragene Punkte
nicht mitgezaehlt wurden. Die 4 unter `[!]` sind aus `[~]` hervorgegangen, sie
sind kein Zuwachs. Aeltere Notiz: zuvor am 2026-08-16 nachgezaehlt, nicht geschaetzt - Zaehlung
...
einzigen Durchgang. Was jetzt noch auf einen Bildschirmtermin wartet, traegt seit dem 2026-09-23
die Marke `[!]` und steht mit Pruefanweisung in `docs/PRUEFLISTE.md`: 5.10,
6.1, 6.16, 6.21. (**6.23 war in dieser Aufzaehlung falsch** — es ist am
2026-08-13 live bestaetigt worden, "3D staedte sieht gut aus", also drei Tage
vor der Zaehlung, die es hier eintrug.)
...
| **[x]** | **16.10** | **ERLEDIGT 2026-09-23: `docs/PRUEFLISTE.md` angelegt.** Nutzervorgabe: "weiterhin wird auf die Prueliste eingetragen was mit umsetzung des tickets einmal geprueft werden soll". Die Liste ist jetzt Pflichtbestandteil des Ticketabschlusses (`CLAUDE.md`), nicht mehr ein Dokument, das nebenher verwaist. Der Verweis in `docs/spezifikation/15_ANZEIGE.md` zeigt wieder dorthin. *Urspruenglicher Befund:* **Es gab keine gueltige Live-Pruefliste mehr.** `docs/archiv/2026-08-27_PRUEFLISTE_LIVE.md` wurde am 2026-09-21 archiviert, ausdruecklich mit dem Vermerk, dass Punkte "erledigt oder ueberholt" sind, "ohne dass das im Dokument nachgefuehrt wurde". Ein Nachfolger existiert nicht. `docs/spezifikation/15_ANZEIGE.md:236-237` verweist inzwischen hierher (auf `docs/OFFENE_PUNKTE.md`, Eintraege markiert als "nur live pruefbar") — **entweder diese Regelung bestaetigen oder eine eigene Liste anlegen.** | 0.5 |
