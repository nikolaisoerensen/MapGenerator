...
class FakeDLM:
    ...
    tatsaechlich benutzt: get_all_data()/set_all_data() je Kategorie plus die
    drei globalen Getter/Setter. Kein Qt, keine Generator-Objekte - genau so
    viel, wie zum Pruefen des Rundlaufs noetig ist (Projektkonvention, siehe
    tests/smoke_test_export_2048.py)."""
...
def _baue_gefuellten_dlm():
    ...
    dlm._stores["terrain"]["heightmap"] = (
        rng.random((37, 41), dtype=np.float64).astype(np.float32) * 1000.0 - 137.123456
    )
    dlm._stores["terrain"]["slopemap"] = rng.random((37, 41)).astype(np.float32)
    dlm._stores["biome"]["biome_map"] = rng.integers(0, 9, size=(37, 41)).astype(np.int32)
    dlm._stores["settlement"]["settlement_list"] = [{"name": "Stadt A", "pos": (3, 4)},
                                                     {"name": "Stadt B", "pos": (10, 2)}]
    dlm._stores["water"]["ocean_outflow"] = 12345.6789
    return dlm
