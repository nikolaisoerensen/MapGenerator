...
from core.vektor_kueste import (MESS_PROFIL_M_JE_ARCHETYP, PROFIL_STELLEN_M,
                                PROFIL_VOLL_M, MESS_FORM_JE_ARCHETYP, MESS_REICHWEITE_M,
                                MESSWERTE_JE_REGION)
...
#     Verdacht "Landmasse der Steppe" war falsch.
...
def _auswerten(gruppen, strecke):
    ...
    # Nur die Zone, die das Kuestenprofil ueberhaupt beansprucht.
    xs = np.linspace(0.0, PROFIL_VOLL_M, 15)
    for (region, typ), profs in sorted(gruppen.items()):
        if len(profs) < 8:
            continue
        soll = MESS_PROFIL_M_JE_ARCHETYP.get(typ)
        if soll is None:
            continue
        P = np.array(profs)
        s_ = strecke[land]
